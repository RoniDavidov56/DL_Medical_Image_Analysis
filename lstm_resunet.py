...
Original file is located at
    https://colab.research.google.com/drive/1y_9MU3y89qdSsQ-7eeRZYSfmfvenwvdv
"""

# LSTM-ResU-Net architecture:

# Replaces standard convolutional blocks with residual blocks.
# This design preserves fine details of cell structures
# and thin boundaries while reducing over-smoothing.
# Improves the separation of touching cells.
...
# Commented out IPython magic to ensure Python compatibility.
# %%writefile Network_Res.py
# 
# ###
# 
# # This code implements a 2D ResU-Net architecture with ConvLSTM layers
...
#     """
#     Basic ResNet block: Conv-BN-ReLU -> Conv-BN + identity (projection if needed) -> ReLU
#     Supports NCHW / NHWC via data_format.
#     """
#     def __init__(self, out_channels: int, stride: int = 1, data_format: str = "NCHW", name=None):
#         super().__init__(name=name)
#         self.out_channels = out_channels
#         self.stride = stride
#         self.df = data_format
#         self.df_keras = 'channels_first' if data_format == "NCHW" else 'channels_last'
#         self.axis_c = 1 if data_format == "NCHW" else -1
# 
#         self.conv1 = KL.Conv2D(out_channels, kernel_size=3, strides=stride, padding="same",
#                                use_bias=False, data_format=self.df_keras)
#         self.bn1   = BatchNormalization(axis=self.axis_c)
#         self.relu  = KL.LeakyReLU()
# 
#         self.conv2 = KL.Conv2D(out_channels, kernel_size=3, strides=1, padding="same",
#                                use_bias=False, data_format=self.df_keras)
#         self.bn2   = BatchNormalization(axis=self.axis_c)
# 
# 
#         self.proj = None
#         self.proj_bn = None
...
#         in_channels = input_shape[1] if self.df == "NCHW" else input_shape[-1]
#         if self.stride != 1 or in_channels != self.out_channels:
#             self.proj = KL.Conv2D(self.out_channels, kernel_size=1, strides=self.stride, padding="same",
#                                   use_bias=False, data_format=self.df_keras)
#             self.proj_bn = BatchNormalization(axis=self.axis_c)
...
# 
# 
# class DownBlock2D_Res(k.Model):
#     """
#     Encoder scale:
#     - One or more ConvLSTM2D layers (return_sequences=True, stateful=True)
#     - Then a stack of ResidualBlock2D
#     - First residual block uses 'stride' (2 for downsample, except deepest), others stride=1
#     Returns:
#       activ_down: [N, T, C, H', W']   (sequence)
#       activ:      [N*T, C, H', W']    (framewise, for skip / feeding next)
#     """
#     def __init__(self, conv_kernels: List[Tuple[int,int]], lstm_kernels: List[Tuple[int,int]],
#                  stride=2, data_format='NCHW'):
#         super().__init__()
#         self.df = data_format
#         self.df_keras = 'channels_first' if data_format == 'NCHW' else 'channels_last'
#         self.axis_c = 1 if data_format == 'NCHW' else -1
...
#                 k.layers.ConvLSTM2D(filters=kout_lstm, kernel_size=kxy_lstm, strides=1,
#                                     padding='same', data_format=self.df_keras,
#                                     return_sequences=True, stateful=True)
#             )
# 
#         # Residual blocks stack
#         self.ResBlocks = []
#         for b_ind, (_kxy_ignore, kout) in enumerate(conv_kernels):
#             blk_stride = stride if b_ind == 0 else 1
#             self.total_stride *= blk_stride
#             self.ResBlocks.append(ResidualBlock2D(out_channels=kout, stride=blk_stride, data_format=self.df))
# 
#     def call(self, inputs, training=None, mask=None):
#         x_seq = inputs
# 
#         # 1) LSTM
#         for cl in self.ConvLSTM:
#             x_seq = cl(x_seq)
# 
#         # 2) batch
#         if self.df_keras == 'channels_first':  # NCHW
#             shp = tf.shape(x_seq)              # [N,T,C,H,W]
#             N, T, C, H, W = shp[0], shp[1], shp[2], shp[3], shp[4]
#             x = tf.reshape(x_seq, [N*T, C, H, W])
#         else:  # NHWC
#             shp = tf.shape(x_seq)              # [N,T,H,W,C]
#             N, T, H, W, C = shp[0], shp[1], shp[2], shp[3], shp[4]
#             x = tf.reshape(x_seq, [N*T, H, W, C])
# 
#         # 3) Residual Blocks
#         out = x
...
# 
#         if self.df_keras == 'channels_first':
#             shp2 = tf.shape(out)               # [N*T, C2, H2, W2]
#             C2, H2, W2 = shp2[1], shp2[2], shp2[3]
#             out_seq = tf.reshape(out, [N, T, C2, H2, W2])
#         else:
#             shp2 = tf.shape(out)               # [N*T, H2, W2, C2]
#             H2, W2, C2 = shp2[1], shp2[2], shp2[3]
#             out_seq = tf.reshape(out, [N, T, H2, W2, C2])
# 
#         return out_seq, out
# 
...
# 
# 
# class UpBlock2D_Res(k.Model):
#     """
#     Decoder scale:
#     - Bilinear upsample (×1 for deepest, then ×2,×2,×2)
#     - Concat with skip
#     - A stack of ResidualBlock2D
#     - Optional 1×1 head for logits if last tuple is (1, num_classes) or return_logits=True
#     """
#     def __init__(self, kernels: List[Tuple[int,int]], up_factor=2, data_format='NCHW', return_logits=False):
#         super().__init__()
#         self.df = data_format
#         self.df_keras = 'channels_first' if data_format == 'NCHW' else 'channels_last'
#         self.axis_c = 1 if data_format == 'NCHW' else -1
...
# 
#         # parse kernels: residual blocks for k>1, optional head if last k==1
#         self.ResBlocks = []
#         self.head = None
#         use_until = len(kernels)
#         if len(kernels) > 0 and kernels[-1][0] == 1:
#             _k_head, c_head = kernels[-1]
#             self.head = KL.Conv2D(c_head, kernel_size=1, strides=1, padding='same',
#                                   data_format=self.df_keras, use_bias=True)
#             use_until = len(kernels) - 1
# 
#         for i in range(use_until):
#             _kxy, kout = kernels[i]
#             self.ResBlocks.append(ResidualBlock2D(out_channels=kout, stride=1, data_format=self.df))
# 
#     def call(self, inputs, training=None, mask=None):
#         x, skip = inputs  # both 4D
#         # upsample x
#         x = k.backend.resize_images(x, self.up_factor, self.up_factor, self.df_keras, interpolation='bilinear')
#         # concat
#         x = tf.concat([x, skip], axis=self.axis_c)
#         # residual stack
#         for rb in self.ResBlocks:
#             x = rb(x, training=training)
#         # optional head
#         if self.return_logits and self.head is not None:
#             return self.head(x)
...
# 
# 
# class ULSTMnet2D_Res(k.Model):
#     """
#     LSTM-ResUNet:
#     - Down: ConvLSTM stack per scale + ResidualBlock2D stack (first with stride=2 except deepest)
#     - Up:   bilinear upsample + concat skip (no attention) + ResidualBlock2D stack (+ 1×1 head)
#     """
#     def __init__(self, net_params, data_format='NCHW', pad_image=True):
#         super().__init__()
#         self.df = data_format
#         self.df_keras = 'channels_first' if data_format == 'NCHW' else 'channels_last'
#         self.axis_c = 1 if data_format == 'NCHW' else -1
...
#             raise ValueError("down_conv_kernels and lstm_kernels must have same length")
#         if not len(net_params['down_conv_kernels']) == len(net_params['up_conv_kernels']):
#             raise ValueError("down_conv_kernels and up_conv_kernels must have same length")
# 
#         self.DownLayers = []
...
#             blk = DownBlock2D_Res(conv_filters, lstm_filters, stride=stride, data_format=self.df)
#             self.DownLayers.append(blk)
#             self.total_stride *= blk.total_stride
# 
#         self.UpLayers = []
...
#             blk = UpBlock2D_Res(conv_filters, up_factor=up_factor, data_format=self.df,
#                                 return_logits=return_logits)
#             self.UpLayers.append(blk)
#             self.last_depth = conv_filters[-1][1]  # last tuple's channels (likely classes)
# 
#         # softmax axis: NCHW logits are [N,T,C,H,W] -> classes at dim=2; NHWC -> dim=-1
#         self.softmax_axis = 2 if self.df == 'NCHW' else -1
#         self.Softmax = k.layers.Softmax(axis=self.softmax_axis)
# 
#     def call(self, inputs, training=None, mask=None):
#         in_shape = inputs.shape
#         min_pad = self.total_stride * int(self.pad_image) if self.pad_image else 0
# 
#         if self.df == 'NCHW':
#             pad_y = [min_pad, min_pad + tf.math.mod(self.total_stride - tf.math.mod(in_shape[3], self.total_stride), self.total_stride)]
#             pad_x = [min_pad, min_pad + tf.math.mod(self.total_stride - tf.math.mod(in_shape[4], self.total_stride), self.total_stride)]
#             paddings = [[0,0],[0,0],[0,0], pad_y, pad_x]
#             crops = [[0, in_shape[0]], [0, in_shape[1]], [0, self.last_depth],
#                      [pad_y[0], pad_y[0] + in_shape[3]], [pad_x[0], pad_x[0] + in_shape[4]]]
#         else:
#             pad_y = [min_pad, min_pad + tf.math.mod(self.total_stride - tf.math.mod(in_shape[2], self.total_stride), self.total_stride)]
#             pad_x = [min_pad, min_pad + tf.math.mod(self.total_stride - tf.math.mod(in_shape[3], self.total_stride), self.total_stride)]
#             paddings = [[0,0],[0,0], pad_y, pad_x, [0,0]]
#             crops = [[0, in_shape[0]], [0, in_shape[1]], [pad_y[0], in_shape[2] + pad_y[0]],
#                      [pad_x[0], in_shape[3] + pad_x[0]], [0, self.last_depth]]
# 
#         x = tf.pad(inputs, paddings, "REFLECT")
#         x_shape = tf.shape(x)
# 
#         # ===== ENCODER=====
#         skips = []
...
#             # collect skip (flatten N*T)
#             if self.df == 'NCHW':
#                 shp = tf.shape(out_down)  # [N,T,C,H,W]
#                 s = tf.reshape(out_down, [shp[0]*shp[1], shp[2], shp[3], shp[4]])
#             else:
#                 shp = tf.shape(out_down)  # [N,T,H,W,C]
#                 s = tf.reshape(out_down, [shp[0]*shp[1], shp[2], shp[3], shp[4]])
#             skips.append(s)
# 
#             # next leyer
#             out_down, out_skip = down(out_down, training=training, mask=mask)
# 
#         # ===== DECODER =====
#         skips.reverse()
#         up = out_skip  # flattened at deepest scale
#         for up_blk, sk in zip(self.UpLayers, skips):
# 
#             up = up_blk((up, sk), training=training, mask=mask)
# 
#         # ===== reshape logits back to sequence =====
#         logits_shape = tf.shape(up)  # 4D
#         if self.df == 'NCHW':
#             # up: [N*T, C, H, W] -> [N,T,C,H,W]
#             N, T = x_shape[0], x_shape[1]
#             logits = tf.reshape(up, [N, T, logits_shape[1], logits_shape[2], logits_shape[3]])
#         else:
#             # up: [N*T, H, W, C] -> [N,T,H,W,C]
#             N, T = x_shape[0], x_shape[1]
#             logits = tf.reshape(up, [N, T, logits_shape[1], logits_shape[2], logits_shape[3]])
# 
#         logits = logits[crops[0][0]:crops[0][1],
#                         crops[1][0]:crops[1][1],
#                         crops[2][0]:crops[2][1],
#                         crops[3][0]:crops[3][1],
#                         crops[4][0]:crops[4][1]]
...
#     def reset_states_per_batch(self, is_last_batch):
#         for down in self.DownLayers:
#             down.reset_states_per_batch(is_last_batch)
# 
#     def get_states(self):
#         return [down.get_states() for down in self.DownLayers]
# 
#     def set_states(self, states):
#         for down, st in zip(self.DownLayers, states):
#             down.set_states(st)
# 
#
...
# import os
# from datetime import datetime
# import Network_Res as Nets  #import Networks as Nets
# 
# __author__ = 'arbellea@post.bgu.ac.il'
# 
# 
...
#     model_path = r'/content/output_model/LSTMUNet/MyRun_SIM/2025-08-15_110212'
...
# 
# 
#             out_fname_bin = mask_out.format(time=t)
...
# PR_DIR   = r"/content/output/IntermediateImages/2025-08-15_134408"
