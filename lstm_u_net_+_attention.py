# -*- coding: utf-8 -*-
"""LSTM-U-Net + Attention.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1twT0NezzSwWaMVmSSwPQlrAD0_nJHyqR
"""

# LSTM-U-Net + Attention:

# Adds spatial attention gates to each encoder–decoder skip connection.
# This enables the decoder to focus on the most relevant regions
# while suppressing background clutter.
# Improves sharpness, preserves boundaries in crowded cell scenes,
# and reduces false positives from background noise.
...
# ###
# 
# # This code implements a 2D U-Net + Attention architecture with ConvLSTM layers
...
# }
# 
# class AttentionGate2D(tf.keras.layers.Layer):
#     def __init__(self, in_channels_x, in_channels_g, inter_channels=None, data_format='NHWC', name=None):
#         super().__init__(name=name)
#         self.data_format = data_format  # 'NCHW' or 'NHWC'
#         axis_c = 1 if data_format == 'NCHW' else -1
#         if inter_channels is None:
#             inter_channels = max(in_channels_x // 2, 1)
# 
#         df_keras = 'channels_first' if data_format == 'NCHW' else 'channels_last'
# 
#         # 1x1 convs
#         self.theta_x = KL.Conv2D(inter_channels, kernel_size=1, padding='same', use_bias=True,
#                                  data_format=df_keras)
#         self.phi_g   = KL.Conv2D(inter_channels, kernel_size=1, padding='same', use_bias=True,
#                                  data_format=df_keras)
#         self.psi     = KL.Conv2D(1, kernel_size=1, padding='same', use_bias=True,
#                                  data_format=df_keras)
# 
#         self.act = KL.Activation('relu')
#         self.sig = KL.Activation('sigmoid')
#         self.mul = KL.Multiply()
#         self.axis_c = axis_c
# 
#     def call(self, inputs, training=False):
#         # x: encoder skip, g: decoder feature
#         x, g = inputs  # 4D tensors
# 
#         theta_x = self.theta_x(x, training=training)  # shape like x
#         phi_g   = self.phi_g(g, training=training)    # shape like g
# 
#         # ---- Always resize g to x's spatial size (no Python if on tensors) ----
#         if self.data_format == 'NCHW':
#             # NCHW -> NHWC to use tf.image.resize, then back
#             x_h = tf.shape(theta_x)[2]
#             x_w = tf.shape(theta_x)[3]
#             phi_g_nhwc = tf.transpose(phi_g, [0, 2, 3, 1])
#             phi_g_nhwc = tf.image.resize(phi_g_nhwc, size=[x_h, x_w], method='bilinear')
#             phi_g = tf.transpose(phi_g_nhwc, [0, 3, 1, 2])
#         else:  # NHWC
#             x_h = tf.shape(theta_x)[1]
#             x_w = tf.shape(theta_x)[2]
#             phi_g = tf.image.resize(phi_g, size=[x_h, x_w], method='bilinear')
# 
#         # attention mask
#         att = self.act(theta_x + phi_g)
#         att = self.psi(att, training=training)
#         att = self.sig(att)
# 
#         # gate skip
#         out = self.mul([x, 1.0 + att])
#         return out
# 
# 
# 
...
#         softmax_axis = 2 if self.channel_axis == 1 else -1  # NCHW -> 2, NHWC -> -1
#         self.Softmax = k.layers.Softmax(axis=softmax_axis)
#     def call(self, inputs, training=None, mask=None):
...
#         skip_inputs = []
#         out_down = inputs
#         out_skip = tf.reshape(inputs, [input_shape[0] * input_shape[1],
...
# 
#         up_input = out_skip
#         skip_inputs.reverse()  # : [H/8, H/4, H/2, H]
# 
#         # affter skip_inputs.reverse()
#         def _ch(t):
#             return t.shape[1] if self.channel_axis == 1 else t.shape[-1]
# 
#         # [512, 256, 256, 128]
#         # print("skip channels (deep->shallow):", [_ch(s) for s in skip_inputs])
# 
# 
#         assert _ch(skip_inputs[0]) == 512, "deep skip should have 512 channels"
#         assert _ch(skip_inputs[1]) == 256, "next skip should have 256 channels"
#         assert _ch(skip_inputs[2]) == 256, "next skip should have 256 channels"
#         assert _ch(skip_inputs[3]) == 128, "shallow skip should have 128 channels"
# 
...
# 
# 
# class ULSTMnet2D_Attend(ULSTMnet2D):
#     def __init__(self, net_params=DEFAULT_NET_DOWN_PARAMS, data_format='NCHW', pad_image=True):
#         super().__init__(net_params=net_params, data_format=data_format, pad_image=pad_image)
#         # channel axis like base
#         self.df_str = data_format  # 'NCHW' or 'NHWC'
#         self.channel_axis = 1 if data_format == 'NCHW' else -1
# 
#         # === Define Attention Gates per scale (match your filters) ===
#         # Skips (encoder) channel counts per scale (deep->shallow):
#         #   from your net_kernel_params down path last convs: [512, 256, 256, 128]
#         # Decoder (gating) channel counts (deep->shallow) just before concat:
#         #   from your up path convs: [512, 256, 128, 64]
#         self.ag4 = AttentionGate2D(in_channels_x=512, in_channels_g=512, inter_channels=256, data_format=self.df_str, name='ag4')
#         self.ag3 = AttentionGate2D(in_channels_x=256, in_channels_g=256, inter_channels=128, data_format=self.df_str, name='ag3')
#         self.ag2 = AttentionGate2D(in_channels_x=256, in_channels_g=128, inter_channels=64,  data_format=self.df_str, name='ag2')
#         self.ag1 = AttentionGate2D(in_channels_x=128, in_channels_g=64,  inter_channels=32,  data_format=self.df_str, name='ag1')
# 
#     def call(self, inputs, training=None, mask=None):
#         # ==== Copy the padding logic from the base class ====
#         input_shape = inputs.shape
#         min_pad_value = self.total_stride * int(self.pad_image) if self.pad_image else 0
# 
#         if self.channel_axis == 1:  # NCHW
...
#         skip_inputs = []
#         out_down = x
#         out_skip = tf.reshape(x, [x_shape[0] * x_shape[1], x_shape[2], x_shape[3], x_shape[4]])
#         for down_layer in self.DownLayers:
#             skip_inputs.append(out_skip)
#             out_down, out_skip = down_layer(out_down, training=training, mask=mask)
# 
...
#         skip_inputs.reverse()  # [H/8, H/4, H/2, H]
# 
#         # Map first pair to deepest gate (ag4), then ag3, ag2, ag1
#         gates = [self.ag4, self.ag3, self.ag2, self.ag1]
#         for i, (up_layer, skip) in enumerate(zip(self.UpLayers, skip_inputs)):
#             # Gate the skip using the current decoder feature as the gate signal
#             gated_skip = gates[i]([skip, up_input], training=training)
#             # Standard UNet concat but with gated skip
#             up_input = up_layer((up_input, gated_skip), training=training, mask=mask)
# 
#         # ==== HEAD (same as base) ====
#         logits_output_shape = up_input.shape
#         logits_output = tf.reshape(
#             up_input,
#             [x_shape[0], x_shape[1], logits_output_shape[1], logits_output_shape[2], logits_output_shape[3]]
#         )
# 
...
#     net_model = Nets.ULSTMnet2D_Attend
#     net_kernel_params = {
#         'down_conv_kernels': [
...
#     model_path = r'/content/output_model/LSTMUNet/MyRun_SIM/2025-08-14_144403'
...
#                 model_fname = os.path.join(params.experiment_save_dir, 'model.weights.h5')
#                 # Ensure model is built before saving (Keras 3 requirement)
#                 if not getattr(model, "built", False):
#                     H, W = params.crop_size
#                     if params.data_format == "NCHW":
#                         dummy = tf.zeros([1, params.unroll_len, 1, H, W], dtype=tf.float32)
#                     else:  # NHWC
#                         dummy = tf.zeros([1, params.unroll_len, H, W, 1], dtype=tf.float32)
#                     _ = model(dummy, training=False)
# 
#                 model.save_weights(model_fname + ".weights")
...
# 
# 
# def find_checkpoint_prefix(dir_path, hint='model.weights'):
# 
#     candidates = [f for f in os.listdir(dir_path) if f.endswith('.index')]
#     if not candidates:
#         raise FileNotFoundError("No checkpoint .index file found in {}".format(dir_path))
# 
#     candidates.sort()
#     chosen = None
#     for f in candidates:
...
#     if chosen is None:
#         chosen = candidates[-1]
#     return os.path.join(dir_path, chosen[:-len('.index')])
# 
# 
# 
# def inference():
...
#         ckpt_prefix = find_checkpoint_prefix(params.model_path, hint='model.weights')
#         model.load_weights(ckpt_prefix)
# #        model.load_weights(os.path.join(params.model_path, 'model.weights.h5.weights'))
# 
# #        log_print("Restored from {}".format(os.path.join(params.model_path, 'model.weights.h5.weights')))
#         log_print(f"Restored from {ckpt_prefix}")
# 
...
# 
# 
#             # image_softmax_np = np.argmax(image_softmax_resized, axis=0)  # (1024,1024)
#             if t < 0:
#                 continue
# 
...
# PR_DIR   = r"/content/output/IntermediateImages/2025-08-14_172827"
